...
@sync_to_async
def active_internments_page(request):
    if request.method == 'GET':
        ...
        context = {
            'datatable_url': '/api/datatables/internments/active/',
            'user': get_user_context(user),
        }

        return render(request, 'internments/html/active_internments.html', context=context)
